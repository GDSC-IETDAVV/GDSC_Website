  ...
  return (
    <div className='mb-20'>
        <div className="grid grid-cols-3 gap-4 font-noto">
            {techStack.map((item) => {
                if (item.id === 5) {
                  const selectedTitle = selected === 0 ? item.title : techStack[selected-1].title as string;
                  const selectedDesc = selected === 0 ? item.description : techStack[selected-1].description as string;
                    return (
                    <MiddleCard title={selectedTitle} desc={selectedDesc} />
                )}
                return (
